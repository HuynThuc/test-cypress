...
import {
  useUpdateActiveHelenSpeaking,
  useUpdateInactiveHelenSpeaking,
} from '../../../../api';
import { IHelenSpeaking } from '../../../../types';
import { helenSpeakingStatuses } from '../../data';

interface DataTableRowActionsProps {
  row: Row<IHelenSpeaking>;
}

export function HelenSpeakingActiveActions({ row }: DataTableRowActionsProps) {
  const [isPopoverOpen, setIsPopoverOpen] = useState(false); // State for popover visibility
  const [isLoading, setIsLoading] = useState(false);

  const { mutate: updateActiveHelenSpeaking } = useUpdateActiveHelenSpeaking({
    onClose: () => {
  ...
  const { mutate: updateInactiveHelenSpeaking } =
    useUpdateInactiveHelenSpeaking({
      onClose: () => {
        setIsPopoverOpen(false);
        setIsLoading(false);
  ...
  const handleUpdateActive = () => {
    setIsLoading(true);
    updateActiveHelenSpeaking(row.original.id ?? '');
  };

  const handleUpdateInactive = () => {
    setIsLoading(true);
    updateInactiveHelenSpeaking(row.original.id ?? '');
  };
